...
export function shouldUseYarn(): void {
  try {
    ...
    console.error(
      "Yarn is necessary for Create Solidity Contract. Install it by following the official documentation:",
    );
    console.log();
...
export function shouldUseYarnWorkspaces(): void {
  ...
  if (result.replace(os.EOL, "") !== "true") {
    console.error(
      "The Yarn Workspaces feature is necessary for Create Solidity Contract. Please enable it by running this command:",
    );
    console.log();
